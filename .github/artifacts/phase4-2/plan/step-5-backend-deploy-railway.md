...
5. **🔴 USER: Configure Railway environment variables:**
   ...
   | `OZ_API_KEY` | `<your-oz-key>` | **User must provide** |
   | `OZ_MODEL_ID` | `anthropic/claude-haiku-4` | Cheapest model |
...
- Blocking steps:
  ...
  - **🔴 Blocked until: OZ_API_KEY provided by user**
...
| Environment variable misconfiguration | Use `.env.production.example` as a checklist. Startup guards catch missing `JWT_SECRET` and `OZ_API_KEY`. |
